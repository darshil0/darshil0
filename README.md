# Darshil Shah | QA Engineering Leader & AI Architect

**Transforming Healthcare Quality through AI-Driven Engineering & Regulatory Compliance**

---

## 📍 Quick Links
[Experience](#-executive-summary) • [Tech Stack](#️-technical-ecosystem--mastery) • [AI Portfolio](#-latest-projects--innovation-portfolio) • [Impact](#-key-achievements) • [Leadership](#-leadership-philosophy) • [Connect](#-connect--collaborate)

---

## 🚀 Executive Summary

**Test Lead | Healthcare QA Specialist | AI Tester | AWS Certified | MBA | 9+ Years Experience**  
**Ex: Accenture, Infosys, Prime Therapeutics, Kaiser Permanente**

Senior QA Leader with a proven track record of transforming delivery for **Fortune 500 healthcare clients** at Accenture. I architect quality ecosystems where AI-driven automation meets stringent regulatory compliance (**HIPAA, FDA, ISO**), preventing patient safety risks while driving measurable business impact.

**2026 Focus:** Advancing AI testing methodologies including model evaluation, prompt engineering, AI agent optimization, and agent-assisted coding to push the boundaries of healthcare QA. Focusing on creating both single model and multi-model agent architectures.

**Current Status:** 🇺🇸 U.S. Citizen | 🚀 Immediate Joiner | 📍 Dallas/Irving, TX (Open to Remote)

### 🎯 GitHub Stats & Activity
![Profile Views](https://komarev.com/ghpvc/?username=darshil0&color=blue)
[![GitHub Followers](https://img.shields.io/github/followers/darshil0?style=social)](https://github.com/darshil0)
[![GitHub Stars](https://img.shields.io/github/stars/darshil0?style=social)](https://github.com/darshil0)

---

## 🛠️ Technical Ecosystem & Mastery

### 💻 Technology Stack

<div align="center">

#### Programming & Scripting
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=postgresql&logoColor=white)

#### AI & Machine Learning
![Anthropic](https://img.shields.io/badge/Anthropic_MCP-181818?style=for-the-badge&logo=anthropic&logoColor=white)
![Google Gemini](https://img.shields.io/badge/Google_Gemini-8E75B2?style=for-the-badge&logo=google&logoColor=white)
![OpenAI](https://img.shields.io/badge/Prompt_Engineering-412991?style=for-the-badge&logo=openai&logoColor=white)

#### Cloud & DevOps
![AWS](https://img.shields.io/badge/AWS_Certified-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Azure](https://img.shields.io/badge/Azure-0078D4?style=for-the-badge&logo=microsoft-azure&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=for-the-badge&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)

#### Testing & Automation
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=for-the-badge&logo=postman&logoColor=white)
![JIRA](https://img.shields.io/badge/JIRA-0052CC?style=for-the-badge&logo=jira&logoColor=white)

#### Web Technologies
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white)

</div>

### 📋 Detailed Skill Matrix

| Category | Skills & Technologies |
| :--- | :--- |
| **Leadership** | Strategic Roadmap Planning, Resource Modeling, Agile/Scrum, Mentorship, Cross-Functional Team Leadership |
| **AI & LLM** | Anthropic MCP, Google Gemini 2.5, Prompt Engineering, AI Agent Architecture, RAG Systems |
| **Automation** | Rest Assured, Postman, Selenium, Java, Python, SQL, TypeScript |
| **Cloud/DevOps** | AWS (Certified), Azure, Microsoft Copilot, MLOps for Gen-AI, CI/CD Pipelines |
| **Compliance** | HIPAA, FDA 21 CFR Part 11, ADA Accessibility, EHR/EMR Integration, ISO Standards |
| **Frameworks** | Regression, SIT, Exploratory, E2E, API, Database, Web Services, Performance Testing |
| **Tools** | JIRA, Confluence, TestRail, HP ALM, SoapUI, Git, Docker |

### 🏅 Badges & Achievements
- 🎖️ **YOLO Badge** - Merged code on first day
- 🦈 **Pull Shark** - Active contributor with quality pull requests
- 🌟 **79 Followers** - Growing professional network
- 📊 **25 Public Repositories** - Active open-source contributor
- 🎓 **MBA** - Business & Healthcare Technology Leadership
- ⏰ **9+ Years Experience** - Ex: Accenture, Infosys, Prime Therapeutics, Kaiser Permanente

---

## 📂 Latest Projects & Innovation Portfolio

### 🤖 AI Agent & Automation Systems

#### [1. Customer Support Agent](https://github.com/darshil0/customer-support-agent)
**Tech:** TypeScript, Google ADK, Java

Production-ready multi-agent orchestration solution built with Google Agent Development Kit. Features complex query handling, enterprise-grade monitoring, and intelligent routing capabilities. Demonstrates advanced agent architecture patterns for scalable customer support automation.

**Key Features:**
- Multi-agent collaboration architecture
- Real-time query processing and routing
- Enterprise-grade error handling and logging
- Scalable microservices design

---

#### [2. Gemini Omni Suite](https://github.com/darshil0/Gemini-Omni-Suite)
**Tech:** TypeScript, React, Google Gemini 2.5

Multi-modal AI workspace featuring an Intelligent Email Agent, Generative Image Editor, and Live Voice Assistant. Built with modern React architecture and features a stunning glassmorphic UI design that showcases production-ready AI integration patterns.

**Key Features:**
- Intelligent email composition and analysis
- Real-time image generation and editing
- Voice-activated AI assistant
- Modern, accessible UI/UX design

---

#### [3. Gemini PDF Retrieval Agent](https://github.com/darshil0/gemini-pdf-retrieval-agent)
**Tech:** TypeScript, RAG Architecture, Google Gemini 2.5

Semantic search engine for PDFs using advanced RAG (Retrieval-Augmented Generation) architecture. Allows users to jump to exact page citations using sophisticated retrieval and ranking algorithms. Perfect for document-heavy industries like healthcare and legal.

**Key Features:**
- Semantic search with context understanding
- Exact page citation and navigation
- Multi-document corpus handling
- Advanced ranking algorithms

---

### 🔬 Quality Engineering & Testing Frameworks

#### [4. AI Evaluation QA Framework](https://github.com/darshil0/AI-Evaluation-QA)
**Tech:** Python, AI Evaluation Frameworks, LLM APIs

A production-grade system for evaluating LLM responses via rubric-based scoring and automated reporting. *Applying enterprise QA principles to the AI frontier.* This framework enables systematic testing of AI model outputs against defined quality criteria.

**Key Features:**
- Automated rubric-based LLM evaluation
- Comprehensive test reporting and analytics
- Integration with multiple LLM providers
- Scalable evaluation pipeline

---

### 🚀 Full-Stack Applications

#### [5. Flight Tracker App](https://github.com/darshil0/flighttrackerapp)
**Tech:** TypeScript, Modern Web APIs

A minimalistic and elegant flight tracking application demonstrating clean architecture principles, responsive design, and real-time data handling. Showcases front-end development skills with TypeScript and modern web technologies.

**Key Features:**
- Real-time flight tracking
- Clean, minimalist UI design
- Responsive mobile-first architecture
- Performance-optimized data handling

---

#### [6. AI Prompts 2025](https://github.com/darshil0/ai-prompts-2025)
**Tech:** HTML, Markdown, Prompt Engineering

A curated collection of production-ready AI prompts and prompt engineering patterns specifically tailored for healthcare and enterprise applications. Includes best practices, templates, and real-world use cases.

**Key Features:**
- Healthcare-specific prompt templates
- Enterprise compliance considerations
- Prompt optimization patterns
- Real-world use case examples

---

## 💼 Professional Journey & Impact

### 🏥 Healthcare QA Leadership
**Test Lead | Healthcare QA Specialist | AI Tester**
*9+ Years Experience*

Led quality transformation initiatives across multiple Fortune 500 healthcare organizations, with specialized focus on EHR/EMR systems, clinical validation, and regulatory compliance.

**Key Organizations:**
- **Accenture** - Led global QA teams for Fortune 500 healthcare clients
- **Infosys** - Healthcare systems integration and testing
- **Prime Therapeutics** - Pharmacy benefits management systems
- **Kaiser Permanente** - Integrated healthcare delivery systems

### 🎓 Education & Credentials
- **MBA** - Business & Healthcare Technology Leadership
- **AWS Certified** - Cloud Practitioner
- **Scrum Basics™ Registered** - Agile Leadership

---

## 🏆 Key Achievements & Impact

### 💰 Cost Optimization & Efficiency
- **40% reduction in patient care costs** through Shift-Left and Risk-Based Testing methodologies
- **30% improvement in patient wait times** by streamlining QA workflows and automation coverage
- **60% reduction in manual validation effort** through AI-powered testing frameworks

### 👥 Leadership & Team Development
- Led and mentored **global teams of 15+ engineers** across multiple time zones at Accenture
- Earned **leadership excellence awards** for team transformation and delivery innovation
- Built scalable QA practices adopted across multiple Fortune 500 healthcare engagements

### 🔒 Regulatory & Compliance Excellence
- Achieved **100% compliance rates** across HIPAA, FDA, and ISO audits for safety-critical systems
- Pioneered **LLM-based clinical validation systems** to detect medical data compliance violations before production
- Created reusable compliance frameworks reducing audit time by 50%

### 🚀 Innovation & Technical Leadership
- **First-to-market AI testing frameworks** for healthcare LLM applications
- Developed proprietary AI agent architectures for clinical documentation validation
- Published internal best practices for AI-powered quality engineering adopted company-wide

---

## 🎓 Certifications & Continuous Learning (2025-2026)

### 🤖 AI & Agent Architecture
- **Introduction to MCP** - Anthropic | Oct 2025
- **AI Fluency Framework & Foundations** - Anthropic | Aug 2025
- **Becoming an AI Agent Architect** - IBM | Jul 2025
- **ChatGPT Prompt Engineering for Developers** - Coursera | Jul 2025
- **Generative AI in Action** - IBM | Jun 2025

### ☁️ Cloud & MLOps
- **AWS Cloud Practitioner** - AWS | May 2025
- **MLOps for Generative AI** - Google | Jun 2025
- **Microsoft Copilot Foundations** - Microsoft | Jun 2025

### 📋 Agile & Leadership
- **Registered Scrum Basics™** - Scrum Inc. | Aug 2025
- **Generative AI for Project Managers** - PMI | Mar 2025

---

## 💡 Leadership Philosophy

**Empathy, Integrity, and Transparent Communication.**

I believe QA is a strategic business enabler, not a cost center. In healthcare, "quality" isn't just a metric—it's patient safety. I build teams that leverage AI to handle the mundane, allowing human expertise to focus on critical safety judgment.

### Core Beliefs:
- Quality engineering prevents patient safety risks and enables regulatory confidence
- Teams perform best when mentored with empathy and equipped with cutting-edge tools
- AI augments human expertise but never replaces human judgment in safety-critical systems
- Continuous learning and innovation are essential in the rapidly evolving AI landscape

---

## 📊 GitHub Activity & Contributions

```text
🌟 Total Stars Earned: 4+
🔄 Active Repositories: 20
👥 Community Connections: 45 followers • 78 following
💻 Primary Languages: TypeScript, Python, Java
🎯 Current Focus: AI Agent Architecture, Healthcare QA Automation
```

---

## 🤝 Connect & Collaborate

<div align="center">

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://linkedin.com/in/darshil-qa-lead)
[![Twitter](https://img.shields.io/badge/X-000000?style=for-the-badge&logo=x&logoColor=white)](https://x.com/soulsurfer300)
[![Email](https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:darshils99@gmail.com)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/darshil0)

</div>

### 📍 Location & Availability
- **Location:** Dallas / Irving, TX (Open to Remote)
- **Status:** 🇺🇸 **U.S. Citizen** | 🚀 **Immediate Joiner**
- **Time Zone:** CST (UTC -06:00)

### 🎯 Seeking Opportunities In:
- **Leadership Roles:** QA Lead, Test Lead, QA Manager, AI Testing Lead
- **Specialized Roles:** AI QA Engineer, Prompt Engineer, MLOps Engineer
- **Healthcare Focus:** Clinical Systems QA, EHR/EMR Testing, Compliance Testing
- **Emerging Tech:** AI Agent Testing, LLM Evaluation, Generative AI QA

---

## 📈 GitHub Stats & Contributions

<div align="center">

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=darshil0&show_icons=true&theme=radical&hide_border=true&bg_color=0D1117&title_color=F85D7F&icon_color=F8D866&text_color=FFFFFF)

![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=darshil0&theme=radical&hide_border=true&background=0D1117&stroke=F85D7F&ring=F85D7F&fire=F8D866&currStreakLabel=FFFFFF)

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=darshil0&layout=compact&theme=radical&hide_border=true&bg_color=0D1117&title_color=F85D7F&text_color=FFFFFF)

### 📊 Contribution Activity
![Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=darshil0&theme=github-dark&hide_border=true&bg_color=0D1117&color=F85D7F&line=F8D866&point=FFFFFF)

</div>

---

## 🔥 Recent Activity

<!--START_SECTION:activity-->
- 🤖 Building advanced AI agent architectures
- 📚 Exploring MCP (Model Context Protocol) integrations
- 🏥 Developing healthcare-specific AI testing frameworks
- 🎓 Completing AI and MLOps certifications
- 💡 Contributing to open-source AI tooling
<!--END_SECTION:activity-->

---

## 💼 What I Bring to Your Team

<details>
<summary><b>🎯 Strategic Leadership</b></summary>

- Global team management across multiple time zones
- Agile/Scrum transformation and optimization
- Resource planning and capacity modeling
- Stakeholder management and executive reporting
</details>

<details>
<summary><b>🤖 AI & Innovation</b></summary>

- AI agent architecture and orchestration
- LLM evaluation and testing frameworks
- Prompt engineering best practices
- MLOps pipeline development
</details>

<details>
<summary><b>🏥 Healthcare Expertise</b></summary>

- HIPAA, FDA, and ISO compliance
- Clinical systems validation
- EHR/EMR integration testing
- Patient safety risk assessment
</details>

<details>
<summary><b>⚙️ Technical Excellence</b></summary>

- Full-stack test automation
- API and microservices testing
- Cloud-native quality engineering
- CI/CD pipeline optimization
</details>

---

<div align="center">

### *"I lead QA teams that do far more than find bugs—we prevent patient safety risks and drive measurable business impact."*

---

**⭐ If you find my work valuable, consider starring my repositories!**

**🤝 Open to collaborations on healthcare AI, testing frameworks, and innovative QA solutions.**

---

![Profile Views](https://komarev.com/ghpvc/?username=darshil0&color=brightgreen&style=flat-square)

**Last Updated:** January 2026

</div>
